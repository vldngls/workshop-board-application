...
export default function MainDashboard() {
  ...
  const todayMetrics = useMemo<TodayMetrics>(() => {
    ...
    // Overdue jobs (past their scheduled time and not completed)
    const now = new Date()
  ...
  if (meStatus !== 'success' || !userRole) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{background: 'var(--ios-bg-secondary)'}}>
        <Toaster position="top-right" />
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-ios-primary border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-ios-text-secondary font-medium">Loading...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 space-y-4" style={{background: 'var(--ios-bg-secondary)'}}>
      ...
      {/* Consolidated Main Metrics Card - Single Card with All Info */}
      ...
      {/* Pending Jobs - Only if exists, compact */}
